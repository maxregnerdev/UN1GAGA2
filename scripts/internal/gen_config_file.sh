...
# GET_BUILD_VAR <name> [default]
# Emits a "name=\"value\"" config line. If the variable is unset and a default
# is supplied, the default is used and emitted; otherwise the variable must be
# set or the build aborts.
GET_BUILD_VAR()
{
    if [ "$2" ]; then
        if [ ! "${!1}" ]; then
...
# Returns "true" when the official UN1CA platform and OTA signing keys are
# present in $SRC_DIR/security, otherwise "false".
IS_UNICA_CERT_AVAILABLE()
{
...
_EMIT_FIRMWARE_ARRAY()
{
    local PREFIX="$1"
    local -n ARR="${PREFIX}_EXTRA_FIRMWARES"
    if [ "${#ARR[@]}" -ge 1 ]; then
        echo "${PREFIX}_EXTRA_FIRMWARES=\"$(IFS=":"; printf '%s' "${ARR[*]}")\""
    else
        echo "${PREFIX}_EXTRA_FIRMWARES=\"\""
    fi
}
